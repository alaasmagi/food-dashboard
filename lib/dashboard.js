...
export async function getDashboard() {
  ...
  const environments = config.environments.map((environment) => ({
    ...environment,
    restaurants: environment.restaurants.filter(hasOffersEnabled)
  }));
...
export async function getWheelConfig() {
  ...
  return {
    environments: config.environments.map((environment) => ({
      ...environment,
      restaurants: environment.restaurants.filter(showOnWheelEnabled)
    }))
  };
}
...
function hasOffersEnabled(restaurant) {
  return restaurant.hasOffers === true;
}

function showOnWheelEnabled(restaurant) {
  return restaurant.showOnWheel === true;
}
...
function restaurantKey(restaurant) {
  return `${CACHE_VERSION}:${restaurant.city}/${normalizeSlugForKey(restaurant.slug)}`.toLowerCase();
}

function encodeRestaurantSlug(slug) {
  return slug.split("/").map((part) => encodeURIComponent(decodeSlugPart(part))).join("/");
}

function normalizeSlugForKey(slug) {
  return slug.split("/").map(decodeSlugPart).join("/");
}
